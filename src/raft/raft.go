...
func (rf *Raft) resetElectionTimeout() {
	electionTimeout := (rand.Intn(200) + 250) * 1e6
	rf.mu.Lock()
	defer rf.mu.Unlock()
		...
		if i != me {
			go func(idx int, oldTerm int) {
				for {
					...
					if rf.leaderId != rf.me || rf.currentTerm != oldTerm {
						rf.mu.Unlock()
						return
					}
					rf.mu.Unlock()
	...
	for i := 0; i < n; i++ {
		if i != me {
			go func(idx int) {
				...
				if reply.Term == rf.currentTerm && reply.VoteGranted && !ok {
